...
export class TrendingComponent implements OnInit {
  ...
  initializeTrending(type: string, timeWindow: string, container: MovieModel[], page: number) {
    this.service.getTrending(type, timeWindow, page).subscribe(moviesEl => {
      moviesEl.results.forEach((moviesEl: MovieModel) => {
        container.push({
          ...
          poster_path: 'https://image.tmdb.org/3/t/p/w500/' + moviesEl.poster_path,
          ...
          backdrop_path: 'https://image.tmdb.org/3/t/p/w500/' + moviesEl.backdrop_path,
