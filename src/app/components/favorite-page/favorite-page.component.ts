...
export class FavoritePageComponent implements OnInit, OnDestroy {
  ...
  initializeDetails() {
    this.movieElement = this.storage
      .subscribeStorageValue<number[]>('favoriteId', [])
      .subscribe((favorite) => {
        const removedIds = Object.keys(this.favoriteMovies)
          .map(Number)
          .filter((e) => !favorite.includes(e));
        for (const removedId of removedIds)
          delete this.favoriteMovies[removedId];
        const ids = favorite.filter((e) => !(e in this.favoriteMovies));
        forkJoin(ids.map((e) => this.http.getDetails('movie', e))).subscribe(
          (movies: DetailsModel[]) => {
            for (let i = 0; i < movies.length; i++)
              this.favoriteMovies[ids[i]] = movies[i];
          },
        );
      });
  }
